	...
	case STATE_GET_HID_DESCRIPTOR: // Configured. We now get HID descriptor to be able to control leds, etc...
		{
			...
			case USBH_PACKET_CALLBACK_STATUS_OK:
				...
				setup_data.wLength = USBH_HID_BUFFER;
	...
	case STATE_GET_HID_DESCRIPTOR_RESPONSE_READ:
		{
			...
			case USBH_PACKET_CALLBACK_STATUS_OK:
				...
				device_xfer_control_read(hid->buffer, USBH_HID_BUFFER, event, dev);
				break;
			...
			case USBH_PACKET_CALLBACK_STATUS_ERRSIZ:
				{
					...
					if (length_to_read > USBH_HID_BUFFER) {
						length_to_read = USBH_HID_BUFFER;
					}
